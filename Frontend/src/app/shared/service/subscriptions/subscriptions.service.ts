...
export class SubscriptionsService {
    ...
    private GET_PUBLIC_SUBSCRIPTIONS = `${this.backendUrl}/api/subscriptions/public/plans`;
    private GET_CURRENT_SUBSCRIPTION = `${this.backendUrl}/api/subscriptions/current-subscription`;
    ...
    getCurrentSubscription(): Observable<any> {
        return this.http.get<any>(`${this.GET_CURRENT_SUBSCRIPTION}`)
    }
    ...
    getPublicSubscriptions(page: number = 1, limit: number = 6): Observable<any> {
        return this.http.get<any>(`${this.GET_PUBLIC_SUBSCRIPTIONS}?page=${page}&limit=${limit}`);
    }
